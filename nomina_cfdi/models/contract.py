...
class Contract(models.Model):
    ...
    def _compute_sueldo(self):
        if self.wage:
            values = {
            'sueldo_diario': self.wage/30.42,
            'sueldo_hora': self.wage/30.42/8,
    ...
    def calculate_sueldo_base_cotizacion(self): 
        if self.date_start: 
            ...
            sdi = ((365 + tablas_cfdi_line.aguinaldo + (tablas_cfdi_line.vacaciones)* (tablas_cfdi_line.prima_vac/100.0) ) / 365.0 ) * self.wage/30.42
            if sdi > max_sdi:
