...
class DiasFeriados(models.Model):
    ...
    def action_validar(self):
        if self.fecha:
            ...
            date_from = fecha.strftime("%Y-%m-%d") + ' 06:00:00'
            date_to = fecha.strftime("%Y-%m-%d") +' 20:00:00'
        else:
       ...
       # timezone = tools.ustr(timezone).encode('utf-8')
        ...
        if not leave_type:
           leave_type = self.env['hr.leave.type'].create({'name': 'DFES', 'limit': True})
        ...
        if registro_falta:
           ...
           feriado = self.env['hr.holidays'].create(vals)
